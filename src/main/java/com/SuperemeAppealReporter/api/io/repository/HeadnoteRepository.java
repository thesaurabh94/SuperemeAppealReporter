...
public interface HeadnoteRepository extends PagingAndSortingRepository<HeadnoteEntity, Integer> {
	...
	/*
	 * @Query(value =
	 * "select h.actname1, h.actname2,h.actname3,h.actname4 from HeadnoteEntity h where upper(h.actname1) like %:actname% OR "
	 * +
	 * "upper(h.actname2) like %:actname% OR upper(h.actname3) like %:actname% OR upper(h.actname4) like %:actname% "
	 * ) public List<String[]> searchActname(@Param("actname")String
	 * actname,Pageable pageableRequest);
	 */
	...
	@Query(value = "select  h.actname1, h.actname2,h.actname3,h.actname4 from HeadnoteEntity h where ( upper(h.actname1) like %:actname% OR "
			+ "upper(h.actname2) like %:actname% OR upper(h.actname3) like %:actname% OR upper(h.actname4) like %:actname% ) and h.active = 1 and h.caseEntity.isLive=1 ", countQuery = " select count(h.actname1) from HeadnoteEntity h where h.caseEntity.isLive=1 and h.active = 1")
